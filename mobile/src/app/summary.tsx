...
function Complete({ split, duration, sets, volume, reps, achievements, exercises, ratings, onFinish }: { split: string; duration: string; sets: number; volume: number; reps: number; achievements: ReturnType<typeof getWorkoutAchievements>; exercises: ReturnType<typeof getWorkoutVisitExercises>; ratings: ReturnType<typeof getWorkoutMuscleRatings>; onFinish: () => void }) {
  ...
  return <View style={[styles.summaryPage, { backgroundColor: colors.background }]}>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.summaryContent}>
      ...
      <View style={styles.sessionStats}><View style={[styles.sessionStat, { backgroundColor: colors.surface }]}><Text style={[styles.sessionStatValue, { color: colors.text }]}>{volume ? `${formatVolume(volume)} lb` : reps}</Text><Text style={[styles.sessionStatLabel, { color: colors.mutedText }]}>{volume ? hasRequiredWeight ? 'Volume' : 'Added volume' : 'Reps'}</Text></View><View style={[styles.sessionStat, { backgroundColor: colors.surface }]}><Text style={[styles.sessionStatValue, { color: colors.text }]}>{sets}</Text><Text style={[styles.sessionStatLabel, { color: colors.mutedText }]}>Sets</Text></View><View style={[styles.sessionStat, { backgroundColor: colors.surface }]}><Text style={[styles.sessionStatValue, { color: colors.text }]}>{exercises.length}</Text><Text style={[styles.sessionStatLabel, { color: colors.mutedText }]}>Exercises</Text></View></View>
      ...
      <View style={styles.exerciseListSection}><Text style={[styles.personalBestTitle, { color: colors.mutedText }]}>Today’s training</Text>{exercises.map((exercise, index) => { const weighted = weightRequiredExerciseIds.has(exercise.id); return <View key={exercise.id} style={[styles.exerciseRow, { borderColor: colors.surfaceStrong, borderBottomWidth: index === exercises.length - 1 ? 0 : 1 }]}><Text style={[styles.exerciseIndex, { color: colors.subtleText }]}>{String(index + 1).padStart(2, '0')}</Text><View style={styles.exerciseCopy}><Text style={[styles.exerciseName, { color: colors.text }]} numberOfLines={1}>{exercise.name}</Text><Text style={[styles.exerciseDetail, { color: colors.mutedText }]}>{exercise.sets} {exercise.sets === 1 ? 'set' : 'sets'}{weighted ? ` · ${formatVolume(exercise.volume)} lb` : exercise.volume ? ` · added weight` : ''}</Text></View></View>; })}</View>
    ...
    <View style={styles.bottomActions}><Pressable onPress={onFinish} style={[styles.primaryButton, { backgroundColor: colors.accent }]}><Text style={[styles.primaryButtonText, { color: colors.accentText }]}>Done</Text></Pressable></View>
  </View>;
}
